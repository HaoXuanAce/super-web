<template>
  <section class="relative flex min-h-full items-start justify-center overflow-hidden rounded-lg bg-stone-50 px-5 py-6 sm:col-span-3 sm:items-center sm:px-7 sm:py-8 md:px-10 lg:col-span-1 xl:px-14">
    <div class="form-grain pointer-events-none absolute inset-0"></div>
    ...
    <div class="relative z-10 w-full max-w-md">
      ...
      <div class="mb-7">
        <p class="text-xs font-semibold uppercase tracking-widest text-amber-700">Sign in to retouch</p>
        <h2 class="mt-3 text-3xl font-semibold tracking-tight text-stone-950 sm:text-4xl">{{ heading }}</h2>
        <p class="mt-3 text-sm leading-6 text-stone-500">{{ subheading }}</p>
      </div>
      ...
      <div class="rounded-lg border border-stone-200 bg-white p-4 shadow-xl shadow-stone-900/5 sm:p-5">
        <div class="mb-5 rounded-lg border border-amber-200 bg-amber-50 p-3">
          <div class="flex items-start gap-3">
            ...
            <div>
              <p class="text-sm font-semibold text-stone-950">{{ modeHighlights[props.mode].title }}</p>
              <p class="mt-1 text-xs leading-5 text-stone-600">{{ modeHighlights[props.mode].description }}</p>
            </div>
          </div>
        </div>
      ...
      <div class="mt-5 grid gap-2 sm:grid-cols-3">
        <div v-for="step in pipelineSteps" :key="step.title" class="rounded-lg border border-stone-200 bg-white/80 p-3 text-xs text-stone-600 shadow-sm backdrop-blur">
          ...
          <p class="font-semibold text-stone-950">{{ step.title }}</p>
          <p class="mt-1 leading-5">{{ step.text }}</p>
        </div>
      </div>
...
const subheading = computed(() => {
  if (props.mode === 'register') return '让提示词、预设和作品都留在你的创作空间里。'
  if (props.mode === 'phone-login') return '输入手机号验证码，快速进入 AI 修图工作台。'
